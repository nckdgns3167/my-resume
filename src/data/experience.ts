        ...
        learningPoints: [
          {
            ...
            paragraphs: [
              "웹에서 설계한 IIFE + Islands Architecture를 Android WebView + NanoHTTPD 위에 그대로 올렸다. 프론트엔드 코드 80%를 재사용할 수 있었던 것은 처음부터 아키텍처를 특정 런타임(브라우저)에 종속시키지 않았기 때문이다.",
          ...
          },
          {
            ...
            paragraphs: [
              ...
              "검사표 저장과 업로드 큐 등록을 하나의 트랜잭션으로 묶고, Canvas→PDF 순서로 의존성 있는 데이터를 순차 업로드하는 설계를 통해 '부분 성공' 상태를 원천 차단했다. 오프라인 동기화는 결국 데이터 일관성 보장 문제이며, 이것은 분산 시스템의 축소판이라는 것을 배웠다.",
            ],
          },
          {
            topic:
            ...
            paragraphs: [
              "128개 테이블의 Oracle DDL→SQLite 변환을 AI로 자동화했지만, 처음부터 잘 된 것은 아니었다. AI에게 단순히 '변환해줘'라고 하면 NVL, DECODE, 시퀀스 같은 Oracle 고유 문법을 제대로 처리하지 못했다.",
              "전환점은 [metric]30개 AI 컨텍스트 문서[/metric]를 체계적으로 구축한 것이었다. 포팅 가이드, 타입 매핑 규칙, 함수 변환 테이블 등 '프로젝트를 이해시키는' 문서를 만들자 AI의 변환 정확도가 비약적으로 올라갔다. AI 활용의 핵심은 프롬프트가 아니라 맥락 설계라는 것을 이 프로젝트에서 확신하게 되었다.",
            ],
          },
          {
            topic:
            ...
            paragraphs: [
              "Android 앱 내부에 Spring 의존성을 가져올 수 없어서, NanoHTTPD 위에 @RestController, @PostMapping 등 어노테이션 기반 라우팅 프레임워크를 자체 구현했다. 기존 웹 프로젝트의 Controller/Service 구조를 그대로 유지하면서도 Spring 없이 동작하는 경량 프레임워크를 만든 것이다.",
              "이 과정에서 Spring의 진짜 가치는 라이브러리나 기능이 아니라, [metric]개발자가 익숙한 컨벤션[/metric]에 있다는 것을 깨달았다. 팀원들이 새로운 패턴을 학습할 필요 없이 기존 Spring 경험을 그대로 활용할 수 있었기 때문에, 온보딩 비용 없이 150+ API 개발에 바로 투입될 수 있었다.",
            ],
          },
          {
            topic:
            ...
            paragraphs: [
              ...
              "CLAUDE.md와 포팅 가이드를 팀 공유 컨텍스트로 구축하고, 커스텀 스킬(front-sync, svn-commit-msg)로 반복 작업을 자동화하자, 경험 차이와 관계없이 팀원 모두가 일관된 품질의 코드를 생산할 수 있었다. 앞으로의 팀 리딩은 '사람에게 무엇을 시킬 것인가'만이 아니라 'AI에게 무엇을 맡기고, 사람은 무엇에 집중할 것인가'를 함께 설계해야 한다는 것을 배웠다.",
            ],
          },
        ],
        highlightBox: {
