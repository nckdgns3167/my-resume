    ...
    learningPoints: [
      {
        ...
        paragraphs: [
          ...
          "가장 큰 도전은 Canvas 위에서의 히트 테스트(6px 임계값으로 마우스 근처 드로잉 감지)와 시간/논리 좌표↔스크린 픽셀 간 좌표 변환이다. 자유곡선은 원시 마우스 포인트가 지그재그로 보이는 문제를 베지어 곡선 보간으로 해결했고, Factory Pattern으로 도구별 설정을 추상화하여 새 도구 추가 시 설정 파일 하나만 작성하면 되는 구조를 만들었다.",
        ],
      },
      {
        topic: "AI + 통계 하이브리드 예측 — LLM 환각 방지 아키텍처",
        paragraphs: [
          "GPT-4o는 \"비트코인 3일 뒤 가격\"같은 수치 예측에서 환각이 심하다. 금융 QA 벤치마크에서 LLM 단독 정확도가 19%에 불과하다는 연구 결과를 바탕으로, LLM은 정성적 분석(\"왜 오를 수 있는가\")만 담당하고 수치 예측은 통계 모델에 위임하는 하이브리드 아키텍처를 설계했다.",
          "[metric]Monte Carlo 시뮬레이션(경로 생성) + Transformer(패턴 인식) + Prophet(계절성)[/metric] 앙상블을 구현할 예정이다. LLM이 10%, 통계 모델이 90%의 예측 가중치를 갖는 구조로, AI의 강점(설명)과 통계의 강점(수치)을 분리하여 신뢰할 수 있는 예측 시스템을 만드는 것이 목표다.",
        ],
      },
      {
        topic: "실시간 WebSocket + 다중 레이어 Canvas 렌더링 최적화",
        paragraphs: [
          "Binance WebSocket으로 실시간 캔들 데이터를 수신하면서, 드로잉 도구·기술 지표·예측 시나리오가 동시에 하나의 Canvas 위에 렌더링된다. 요소가 늘어날수록 렌더링 병목이 발생할 수 있어, 이벤트 디바운싱과 단일 패스 렌더링(한 번의 Canvas draw 호출에 모든 드로잉을 처리)으로 성능을 관리하고 있다.",
      ...
      },
      {
        ...
        paragraphs: [
          "chart·drawing·analysis·session·onboarding 5개 독립 Zustand 스토어가 각자의 관심사를 담당한다. 가장 복잡한 부분은 세션 전환이다. 사용자가 세션을 바꾸면 차트 심볼·드로잉 목록·대화 히스토리·예측 시나리오를 모든 스토어에서 원자적으로 복원해야 한다.",
          "드로잉 변경 → 세션 스토어 자동 동기화 → localStorage 영속화까지 단방향 데이터 흐름을 설계했고, Undo/Redo 히스토리는 devtools 오염을 방지하기 위해 스토어 외부에서 별도 관리한다. Supabase로 서버 영속화를 도입하면 동기화 복잡도가 한 단계 더 올라갈 예정이다.",
        ],
      },
      {
        ...
        paragraphs: [
          ...
          "각 시나리오를 클릭하면 상세 리포트 모달(7개 섹션)이 열리고, 확률 가중치 기반으로 경로의 투명도를 차등 적용하여 신뢰도를 시각적으로 전달하는 것이 최종 목표다.",
        ],
      },
    ],
    gallery: [
