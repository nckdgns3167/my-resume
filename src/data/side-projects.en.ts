		...
		learningPoints: [
			{
				...
				paragraphs: [
					...
					"The biggest challenges are [metric]hit testing on Canvas[/metric] (detecting drawings near mouse cursor with a 6px threshold) and [metric]coordinate transformation between time/logical and screen pixels[/metric]. Solved the freehand curve's raw mouse point zigzag issue with Bézier curve interpolation, and abstracted per-tool configuration via Factory Pattern so adding a new tool only requires writing one config file.",
				],
			},
			{
				topic: "AI + Statistics Hybrid Prediction — Anti-hallucination Architecture",
				paragraphs: [
					"GPT-4o hallucinates severely on numeric predictions like \"Bitcoin price in 3 days.\" Based on research showing LLM-only accuracy at just 19% on financial QA benchmarks, designed a hybrid architecture [metric]delegating numeric prediction to statistical models[/metric] while LLM handles only [metric]qualitative analysis[/metric] (\"why it might rise\").",
					"Planning to implement a [metric]Monte Carlo simulation (path generation) + Transformer (pattern recognition) + Prophet (seasonality)[/metric] ensemble. The architecture gives LLM 10% and statistical models 90% of prediction weight, [metric]separating AI's strength (explanation) from statistics' strength (numbers)[/metric] to build a trustworthy prediction system.",
				],
			},
			{
				topic: "Real-time WebSocket + Multi-layer Canvas Rendering Optimization",
				paragraphs: [
					"Receiving real-time candle data via Binance WebSocket while drawing tools, technical indicators, and prediction scenarios render simultaneously on a single Canvas. As elements grow, rendering bottlenecks can occur — managing performance through event debouncing and [metric]single-pass rendering[/metric] (processing all drawings in one Canvas draw call).",
			...
			},
			{
				...
				paragraphs: [
					"Five independent Zustand stores — chart, drawing, analysis, session, onboarding — each own their concern. The most complex part is session switching: when a user switches sessions, chart symbol, drawing list, conversation history, and prediction scenarios must be [metric]atomically restored[/metric] across all stores.",
					"Designed a [metric]unidirectional data flow[/metric] from drawing changes → session store auto-sync → localStorage persistence. Undo/Redo history is managed outside stores to prevent devtools pollution. Introducing Supabase server persistence will add another layer of synchronization complexity.",
				],
			},
			{
				...
				paragraphs: [
					...
					"The final goal is to open a detailed report modal (7 sections) on scenario click, and [metric]apply differential transparency to paths based on probability weights[/metric] to visually convey confidence levels.",
				],
			},
		],
		gallery: [
