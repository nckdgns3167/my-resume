...
export function ProjectCard({ project, ui }: ProjectCardProps) {
  return (
    <div id={project.id} className="relative scroll-mt-20">
      {/* 타임라인 작은 도트 */}
      <div className="absolute -left-7 top-[30px] flex h-[15px] w-[15px] items-center justify-center">
        <div className="h-[7px] w-[7px] rounded-full bg-border bg-accent-primary!" />
      </div>

      {/* 프로젝트 카드 — 좌측 accent 보더 + 배경 */}
      <div className="rounded-xl border border-border bg-surface p-6">
        ...
        <div className="mb-3 flex flex-wrap gap-1.5">
          {project.stack.map((tech) => (
            <span
              key={tech}
              className="rounded border border-border bg-bg px-2 py-0.5 text-xs text-text-secondary"
            >
              {tech}
            </span>
          ))}
        ...
        {project.gallery.length > 0 && (
          <div className="mt-5 border-t border-border pt-4 print:hidden">
            <ProjectGallery grids={project.gallery} projectId={project.id} projectName={project.name} />
          </div>
        )}
      </div>
    </div>
